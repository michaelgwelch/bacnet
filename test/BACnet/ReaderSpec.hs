...
spec = do
  ...
  describe "readDoubleAP" $
    it "reads [0x55, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xaa] and returns Just(4.9e-324, [0xaa])" $
      runReader readDoubleAP [0x55, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0xaa]
        `shouldBe` Just(4.9e-324, [0xaa])
